#!/usr/bin/env bash
# Launch the R-arm init joint calibration tool (robot + floor, omni.ui sliders).
...
if [[ -z "${DISPLAY:-}" ]]; then
  echo "[calibrate_r_arm] ERROR: DISPLAY is not set; this tool needs a GUI." >&2
  exit 1
fi
...
exec uv run python task/calibrate_r_arm_joints.py "$@"
