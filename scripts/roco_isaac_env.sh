# Shared Isaac Sim process environment for RoCo launchers.
# Source from repo scripts after setting script_dir/repo_root if needed.
...
_roco_has_glibcxx_3429() {
  local lib="$1"
  [[ -f "$lib" ]] && grep -aq 'GLIBCXX_3.4.29' "$lib"
}

if ! _roco_has_glibcxx_3429 /lib/x86_64-linux-gnu/libstdc++.so.6 \
  && ! _roco_has_glibcxx_3429 /usr/lib/x86_64-linux-gnu/libstdc++.so.6; then
  _libstdcxx=""
  for cand in \
    "${ROCO_LIBSTDCXX:-}" \
  ...
  do
    if [[ -n "$cand" ]] && _roco_has_glibcxx_3429 "$cand"; then
      _libstdcxx="$cand"
      break
    fi
  done
