# ruff: noqa: E402
"""Standalone Isaac Sim tool: calibrate R_arm_j1..j7 init joint targets.

Loads robot + ground only (no table / task board / parts). An omni.ui panel
drives each R-arm joint; Save writes param_config.INIT_JOINT_TARGETS and
keeps Lula descriptor YAMLs in sync.

Launch:
    ./scripts/calibrate_r_arm.sh
    # or: uv run python task/calibrate_r_arm_joints.py
"""
...
if _HEADLESS or not os.environ.get("DISPLAY"):
    print(
        "[r_arm_calib] ERROR: this tool needs a GUI display. "
...
from r_arm_calib_ui import (
    R_ARM_JOINTS,
    RArmCalibUI,
    print_targets,
    save_r_arm_init_targets,
)
...
def _seed_r_targets_from_config() -> dict:
...
def main() -> None:
    ...
    # Hold every non-R DOF at the post-init snapshot; R comes from the UI.
    hold = {
        i: float(q0[i])
        for i, jname in enumerate(dof_names)
        if jname not in R_ARM_JOINTS
    }
    r_indices = {jname: dof_names.index(jname) for jname in R_ARM_JOINTS}
    ...
    initial_r = _seed_r_targets_from_config()

    def _on_save(targets: dict) -> None:
        touched = save_r_arm_init_targets(targets)
        print_targets(targets)
        print("[r_arm_calib] wrote:", flush=True)
        for p in touched:
            print(f"[r_arm_calib]   {p}", flush=True)

    ui = RArmCalibUI(
        initial_targets_rad=initial_r,
        on_save=_on_save,
        on_print=print_targets,
    )
    print(
        "[r_arm_calib] ready — use the 'R Arm Init Calibration' window. "
    ...
    while simulation_app.is_running():
        ...
        for i, val in hold.items():
            q[i] = val
        for jname, idx in r_indices.items():
